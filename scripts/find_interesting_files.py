...
def find_interesting_files(self, target_path,max_depth=5,current_depth=0):
    import os, stat
    import pwd
    import grp
    import time
    ...
    if os.path.isdir(target_path):
        for filename in sorted(os.listdir(target_path)):
            try:
                ...
                if sgid_set:
                    sgid = " *SGID"
                if writable != "" or suid != "" or sgid != "":
                    outstr += "{} {:<5} {:<10} {:<10} {:<13} {:<10} {}{}{}{}\n".format(filetype, permissions, owner, group, file_time, file_size , filepath, writable, suid, sgid)
                if os.path.isdir(filepath) and current_depth < max_depth:
                    outstr += self.find_interesting_files(filepath,max_depth=max_depth,current_depth=current_depth+1)
